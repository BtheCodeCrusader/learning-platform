...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try (SessionFactory factory = buildSessionFactory()) {
            ...
            // OUTPUT FILES
            // =====================
            executeInTransaction(factory, FileProcessor::createOutputFiles);
            System.out.println("✅ Output-Dateien erstellt!");

            System.out.println("\n✅ Gesamte Verarbeitung abgeschlossen!");
    ...
    private static void createSubmission(
        ...
        SubmissionId id =
                new SubmissionId(st.getId(), a.getId());
    ...
    private static void createTestData(Session session) {
        ...
        // Professoren
        Professor prof1 = createProfessor(
                session,
                "Prof. Mustermann",
                "prof@uni.de",
                "Dr.");

        Professor prof2 = createProfessor(
                session,
                "Prof. Schneider",
                "schneider@uni.de",
                "Prof. Dr.");
        ...
        Assignment a4 = createAssignment(
                session,
                "Web Projekt",
                "Mini Webseite",
                14,
        ...
        // Kategorien
        Category cat1 =
                createCategory(session, "Programmierung");

        Category cat2 =
                createCategory(session, "Datenbanken");

        Category cat3 =
                createCategory(session, "Web");
    ...
    private static void runQueries(Session session) {
        ...
        session.createQuery(
                        "SELECT c.title, COUNT(e.student) " +
                                "FROM Enrollment e " +
                                "JOIN e.course c " +
                                "GROUP BY c.title",
                        Object[].class)
        ...
        session.createQuery(
                        "SELECT p.name, AVG(s.grade) " +
                                "FROM Submission s " +
                                "JOIN s.assignment.course c " +
                                "JOIN c.professor p " +
                                "GROUP BY p.name",
                        Object[].class)
                .getResultList()
                .forEach(r ->
                        System.out.println(
                                r[0] + ": "
                                        + String.format("%.1f", r[1])));

        System.out.println("\nZertifikate pro Kurs:");

        session.createQuery(
                        "SELECT c.title, COUNT(cert) " +
